...
def generate_data(num_workers, num_days, num_shifts, num_tracks, num_skills, mean_availability=.9, 
    ...
    schedule_type = f"{num_workers}-{num_days}-{num_shifts}-{num_tracks}-{num_skills}-{mean_availability}-{mean_staffing_requirement}"
...
def generate_worker_data(num_workers, num_days, num_shifts, num_skills, worker_shift_availability_bernoulli_mean):
    ...
    max_hours = total_shifts - np.round(lognorm.rvs(1, loc=0, scale=1, size=num_workers))
    max_hours = np.where(max_hours <= 0, 0, max_hours)
    worker_data_dict["worker_max_shifts"] = max_hours.tolist()
...
def generate_shift_data(num_workers, num_days, num_tracks, num_shifts, num_skills, mean_staffing_requirement):
    ...
    req = np.where(req > num_workers, num_workers, req)
